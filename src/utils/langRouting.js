...
export const getPathForLocale = (lang, logicalPath) => {
  ...
  if (path.replace(/\/+$/, "") === "/careers/partner") {
    return "/lavora-con-noi-partner" + trailing
  }
  ...
  const rest = segments.slice(1).join("/")
  return "/" + replacement + (rest ? `/${rest}` : "") + trailing
}
...
export const getLogicalPathFromLocalizedPath = (path, lang) => {
  ...
  if (first === "lavora-con-noi-partner") {
    return "/careers/partner" + trailing
  }
