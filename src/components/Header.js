...
function Header({ handleLanguage, currentLanguage, handleCountry, currentCountry = "Country", onHeightChange }) {
  ...
  const isItalyArea = currentLang === "it"

  const headerPhoneNumber = isItalyArea ? "+39 331 800 7652" : "+96103322811"
  const headerEmail = isItalyArea ? "inquiries@achiscaffolding.com" : "achi.gr@hotmail.com"
  ...
  const manualOverride = useMemo(() => {
    try {
      return (localStorage.getItem("site_locale_override_v1") || "").toUpperCase()
    } catch {
      return ""
    }
  }, [location.pathname])
  ...
  const displayCountry = useMemo(() => {
    if (isItalyArea) return "Italy"
    if (manualOverride === "IT") return "Italy"
    if (manualOverride === "LB") return "Lebanon"
    if (currentCountry && currentCountry !== "Country") return currentCountry
    return "Lebanon"
  }, [isItalyArea, manualOverride, currentCountry])
  ...
  const handleCountryChange = (country) => {
    ...
    try {
      localStorage.setItem("site_locale_override_v1", manualCountry)
      localStorage.setItem("site_country_lock_v1", manualCountry)
      localStorage.setItem("selectedLanguage", lang)
    } catch {}
  ...
  { key: "services", type: "navlink", to: buildPathWithLang(urlLang, "/services"), label: t("nav.services"), onClick: closeAllDropdowns },
  {
    key: "sectors",
    type: "navlink",
    to: buildPathWithLang(urlLang, "/sectors"),
    label: t("nav.sectors"),
          ...
          <div className="flex flex-row items-center gap-[20px] md:gap-[20px] lg:gap-[20px] xl:gap-[20px] flex-nowrap">
            <div className="inline-flex items-center gap-[8px]">
              <ImageWebp
                ...
                alt="emailIcon"
              />
              <span className="text-[14px] text-white font-[500] leading-[1] font-saira whitespace-nowrap inline-block">
                {headerEmail}
              </span>
            </div>
          ...
          <div className="flex items-center gap-[18px] relative flex-nowrap">
            <div ref={countryDropdownRef} className="relative inline-flex">
              <button
                ...
                className="inline-flex items-center gap-[8px] rounded-lg py-2 px-2 -my-2 -mx-2 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-[#28509E] cursor-pointer hover:bg-white/10"
              ...
              >
                ...
                <span className="text-white font-saira font-[500] text-[14px] leading-[1] whitespace-nowrap">
                  {displayCountry}
                </span>
                <ExpandMoreIcon
                  fontSize="small"
              ...
              <div
                ...
                className={`absolute right-0 top-[calc(100%+8px)] z-[999] min-w-[140px] rounded-xl bg-white shadow-xl ring-1 ring-black/5 overflow-hidden transition-[opacity,transform] duration-200 ease-out origin-top-right ${
                  showCountry ? "opacity-100 scale-100 translate-y-0 pointer-events-auto" : "opacity-0 scale-95 -translate-y-1 pointer-events-none"
                }`}
              >
                <div className="py-2">
                  <button
            ...
            <div className="inline-flex items-center gap-[8px]" role="group" aria-label="Language selector">
              {isItalyArea ? (
                <button
                  ...
                  aria-label={t("header.langItalian")}
                  aria-current={currentLang === "it" ? "true" : "false"}
                  className={`lang-btn ${currentLang === "it" ? "is-active" : ""}`}
                >
                  IT
                </button>
              ) : (
          ...
          <div className="w-full flex justify-center">
            <ul
              className={`flex gap-8 ${
                isHome
                  ? "py-[18px] px-[20px] md:px-[40px] lg:px-[60px] xl:px-[80px] 2xl:px-[100px]"
                  : "py-[12px] px-[20px] md:px-[40px] lg:px-[60px] xl:px-[80px] 2xl:px-[100px]"
              }`}
              dir={isArabic ? "rtl" : "ltr"}
            >
...
>
  <p className="font-[500] text-[20px] font-saira py-2">{t("nav.sectors")}</p>
        ...
        <li className="ltr:ml-[20px] rtl:mr-[20px]">
          <p className="font-[500] text-[20px] cursor-pointer hover:text-[#FA7800] transition duration-500 font-saira py-2" onClick={() => goToHomeSection("clients")}>
            {t("nav.clients")}
          </p>
        </li>
        ...
        <li className="ltr:ml-[20px] rtl:mr-[20px] border-[#FFFFFF] border-solid border-b-[2px] pb-[30px]">
          <p className="font-[500] text-[20px] cursor-pointer hover:text-[#FA7800] transition duration-500 font-saira py-2" onClick={() => goToHomeSection("contactForm")}>
            {t("nav.contact")}
          </p>
        </li>
        ...
        <li className="ltr:ml-[20px] rtl:mr-[20px] mt-[30px] pt-[20px] border-t-[#FFFFFF] border-t-[1px] border-solid">
          <p className="font-[500] text-[18px] font-saira mb-[16px] text-white/90">Country</p>
          <div className="flex flex-col gap-[10px]">
            <button
              type="button"
              onClick={() => handleCountryChange("Lebanon")}
              className={`text-left font-saira text-[18px] ${displayCountry === "Lebanon" ? "text-[#FA7800]" : "text-white"}`}
            >
              Lebanon
            </button>
            {ITALY_ENABLED ? (
              <button
                type="button"
                onClick={() => handleCountryChange("Italy")}
                className={`text-left font-saira text-[18px] ${displayCountry === "Italy" ? "text-[#FA7800]" : "text-white"}`}
              >
                Italy
              </button>
            ) : null}
